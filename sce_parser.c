...
Sce_Ast_Node* build_sce_ast_module(Sce_Lexer_Token_Lists* sce_lexer_token_lists) {
	...
	Sce_Ast_Node* block_ = NULL;
	Sce_Ast_Node* dst_ = NULL;
	Sce_Ast_Node* func_decl_ = NULL;
	Sce_Ast_Node* func_decl_tail_ = NULL;
	...
	while (sce_lexer_token_lists->token_index < sce_lexer_token_lists->token_size) {
		...
		left_->left = NULL;
		set_up_sce_ast_node(left_, NULL, 0, E_Sce_Ast_Global_Modules_Statement);

		if (right_) {
			left_->right = right_;

			if (is_sce_ast_type(right_, E_Sce_Ast_Expr)) {

				if (!filed_expr_) {

					filed_expr_ = filed_expr_tail_ = left_;
				}
				else {
			...
			else if (is_sce_ast_type(right_, E_Sce_Ast_Function_Decl)) {

				if (!func_decl_) {

					func_decl_ = func_decl_tail_ = left_;
				}
				else {
					func_decl_tail_->left = left_;
					func_decl_tail_ = left_;
				}
				/*
				if (!tmp_decl_) {
					if (!dst_) {
					...
					Sce_Ast_Node* ast_ = tmp_decl_->left;
					tmp_decl_->left = left_;
					left_->left = ast_;
					if (tmp_decl_ == block_) block_ = left_;
					else tmp_decl_ = left_;
				}
				*/
			}
			else {
...
Sce_Ast_Node* build_sce_ast_loop_in_statement(Sce_Lexer_Token_Lists* sce_lexer_token_lists) {
	...
	if ((left_ = build_ast_sce_block(sce_lexer_token_lists))) return left_;
	if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Punchcute, ";"))	return build_sce_ast_null_statement(sce_lexer_token_lists);
	if ((left_ = build_sce_ast_loop_in_keyword(sce_lexer_token_lists))) return left_;
...
Sce_Ast_Node* build_sce_ast_base_statement(Sce_Lexer_Token_Lists* sce_lexer_token_lists) {
	...
	if ((left_ = build_sce_ast_while(sce_lexer_token_lists))) return left_;
...
Sce_Ast_Node* build_sce_ast_while(Sce_Lexer_Token_Lists* sce_lexer_token_lists) {
	if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Keyword, "while")) {
		return build_sce_ast_while_process(sce_lexer_token_lists);
	}
	return NULL;
}
Sce_Ast_Node* build_sce_ast_while_process(Sce_Lexer_Token_Lists* sce_lexer_token_lists) {
	...
	if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Punchcute, "(")) {
		...
		if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Punchcute, ")")) {
			if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Punchcute, "{")) {
				right_->data.ast_type = E_Sce_Ast_While;
				right_->left = left_;
				right_->data.buf = NULL;

				int error_code;
				Sce_Ast_Node* block_ = build_sce_ast_loop_process(&error_code, sce_lexer_token_lists, E_Sce_Ast_While_Section);
				right_->right = block_;
				if (error_code == 0) return right_;
				else {
					Sce_Ast_Node* rleft_ = smart_malloc(Sce_Ast_Node, 1);
			...
			right_->left = NULL;
			right_->right = left_;
			SET_ERROR_NODE(right_, "error : extpect to ')' after while condition.");
			return right_;
		}
	}
	SET_ERROR_NODE(right_, "error : extpect to '(' after while.");
	right_->left = NULL;
	right_->right = NULL;
	return right_;
}
	...
	if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Punchcute, "(")) {
		...
		if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Punchcute, ")")) {
			if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Punchcute, "{")) {
				...
				int error_code;
				Sce_Ast_Node* block_ = build_sce_ast_loop_process(&error_code, sce_lexer_token_lists, if_else_type == E_Sce_Ast_If ? E_Sce_Ast_If_Section : E_Sce_Ast_Else_If_Section);
				right_->right = block_;
				if (error_code == 0) return right_;
				else {
					Sce_Ast_Node* rleft_ = smart_malloc(Sce_Ast_Node, 1);
...
Sce_Ast_Node* build_sce_ast_loop_in_else(Sce_Lexer_Token_Lists* sce_lexer_token_lists) {
	...
	if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Punchcute, "{")) {
		...
		int error_code;
		Sce_Ast_Node* block_ = build_sce_ast_loop_process(&error_code, sce_lexer_token_lists, E_Sce_Ast_Else_Section);
		right_->right = block_;
		if (error_code == 0) return right_;
		else {
			Sce_Ast_Node* rleft_ = smart_malloc(Sce_Ast_Node, 1);
...
Sce_Ast_Node* build_sce_ast_loop_in_if(Sce_Lexer_Token_Lists* sce_lexer_token_lists) {
	if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Keyword, "if")) {
		return build_sce_ast_loop_in_if_else(sce_lexer_token_lists, E_Sce_Ast_If);
	}

	else if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Keyword, "else")) {
		if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Keyword, "if")) return build_sce_ast_loop_in_if_else(sce_lexer_token_lists, E_Sce_Ast_Else_If);
		return build_sce_ast_loop_in_else(sce_lexer_token_lists);
	}
}
...
Sce_Ast_Node* build_sce_ast_loop_in_keyword(Sce_Lexer_Token_Lists* sce_lexer_token_lists) {
	Sce_Ast_Node* left_;
	if ((left_ = build_sce_ast_base_keyword(sce_lexer_token_lists))) return left_;
	if (left_ = build_sce_ast_loop_in_if(sce_lexer_token_lists)) return left_;
	if (left_ = build_sce_ast_while(sce_lexer_token_lists)) return left_;
...
Sce_Ast_Node* build_sce_ast_loop_process(int* error_code, Sce_Lexer_Token_Lists* sce_lexer_token_lists, Sce_Ast_Type sce_ast_block_type) {
	...
	while ((*error_code = get_end_block_process(sce_lexer_token_lists)) > 0) {
		if (block_->right) {
			Sce_Ast_Node* left_ = smart_malloc(Sce_Ast_Node, 1);
			if (!left_) break;
			Sce_Ast_Node* right_ = build_sce_ast_loop_in_statement(sce_lexer_token_lists);
			left_->left = NULL;
			left_->right = right_;
			...
			block_->right = build_sce_ast_loop_in_statement(sce_lexer_token_lists);
		}
	}
	return dst_;
}
...
Sce_Ast_Node* build_sce_ast_base_block_process(int* error_code, Sce_Lexer_Token_Lists* sce_lexer_token_lists, Sce_Ast_Type sce_ast_block_type) {
	...
	while ((*error_code = get_end_block_process(sce_lexer_token_lists)) > 0) {
		if (block_->right) {
			Sce_Ast_Node* left_ = smart_malloc(Sce_Ast_Node, 1);
			if (!left_) break;
			Sce_Ast_Node* right_ = build_sce_ast_base_statement(sce_lexer_token_lists);
			left_->left = NULL;
			left_->right = right_;
			...
			block_->right = build_sce_ast_base_statement(sce_lexer_token_lists);
		}
	}
	return dst_;
}
...
Sce_Ast_Node* build_sce_ast_function_args(Sce_Lexer_Token_Lists* sce_lexer_token_lists) {
	Sce_Ast_Node* left_ = NULL;
	Sce_Ast_Node* dst_ = NULL;
	/*
	smart_malloc(Sce_Ast_Node, 1);
	if (!left_)return NULL;
	left_->left = left_->right = NULL;
	...
	*/
	do {
		...
		if (sce_lexer_token->sce_token_type == E_Sce_Lexer_Iden) {
			if (left_) {
				...
				if (!right_) break;
				right_->left = right_->right = NULL;
				left_->left = right_;
				set_up_sce_ast_node(right_, sce_lexer_token->sce_token_str.str__, sce_lexer_token->sce_token_str.index__ + 1, E_Sce_Ast_Parameter_Args_Decl);
				left_ = right_;
			}
			else {
				...
				if (!left_) break;
				left_->left = left_->right = NULL;
				set_up_sce_ast_node(left_, sce_lexer_token->sce_token_str.str__, sce_lexer_token->sce_token_str.index__ + 1, E_Sce_Ast_Parameter_Args_Decl);
				dst_ = left_;
			}
			consume_sce_token_lists(sce_lexer_token_lists);
	...
	} while (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Operator, ","));
	...
	if (sce_lexer_token->sce_token_type == E_Sce_Lexer_Iden) {
		...
		if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Operator, "=")) {
			...
			if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Punchcute, ")")) {
				...
				left_->right = build_sce_ast_function_args(sce_lexer_token_lists);
...
Sce_Ast_Node* build_sce_ast_memebers(Sce_Lexer_Token_Lists* src_ans_token_list) {
	Sce_Ast_Node* left_ = build_sce_ast_array_access(src_ans_token_list);
	while (expect_advance_consume_sce_token_lists(src_ans_token_list, E_Sce_Lexer_Operator, ".")) {
		Sce_Ast_Node* right_ = smart_malloc(Sce_Ast_Node, 1);
		if (!right_)break;
		right_->left = left_;
		...
		right_->right = build_sce_ast_array_access(src_ans_token_list);
		left_ = right_;
	}
	return left_;
}
...
Sce_Ast_Node* build_sce_ast_array_access(Sce_Lexer_Token_Lists* sce_lexer_token_lists) {
	Sce_Ast_Node* left_ = build_sce_ast_call_functions(sce_lexer_token_lists);
	while (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Operator, "[")) {
		Sce_Ast_Node* right_ = smart_calloc(Sce_Ast_Node, 1);
		if (!right_)break;
		right_->left = left_;
		right_->right = build_sce_ast_expr_conmma(sce_lexer_token_lists);

		const char array_error[] = "error : expect to ']'";

		if (expect_advance_consume_sce_token_lists(sce_lexer_token_lists, E_Sce_Lexer_Operator, "]")) 
			set_up_sce_ast_node(right_, NULL, 0, E_Sce_Ast_List_Array_Access);
		/*none is err*/
		else
			set_up_sce_ast_node(right_, array_error, sizeof(array_error), E_Sce_Ast_Error);
...
Sce_Ast_Node* build_sce_ast_array_literal(Sce_Lexer_Token_Lists* src_ans_token_list) {
	/**
	* 
	* i = {1,1,1,1};
	* 
	*/
	...
	if (!left_) return NULL;
	left_->left = NULL;
	set_up_sce_ast_node(left_, NULL, 0, E_Sce_Ast_List_Array_Value);

	if (expect_advance_consume_sce_token_lists(src_ans_token_list, E_Sce_Lexer_Punchcute, "}")) {
		left_->right = NULL;
		return left_;
	}
	left_->right = build_sce_ast_assigment(src_ans_token_list);
	Sce_Ast_Node* tail_ = left_;
	while (expect_advance_consume_sce_token_lists(src_ans_token_list, E_Sce_Lexer_Operator, ",")) {
		if (expect_advance_consume_sce_token_lists(src_ans_token_list, E_Sce_Lexer_Punchcute, "}")) {
			return left_;
		}
		...
		right_->left = NULL;
		right_->right = build_sce_ast_assigment(src_ans_token_list);
		set_up_sce_ast_node(right_, NULL, 0, E_Sce_Ast_List_Array_Value);
		tail_ = right_;
	}
	static const uint8_t error_array[] = "error : expect to '}'.";
	if (!expect_advance_consume_sce_token_lists(src_ans_token_list, E_Sce_Lexer_Punchcute, "}")){
		set_up_sce_ast_node(left_, error_array, sizeof(error_array), E_Sce_Ast_Error);
...
Sce_Ast_Node* build_sce_ast_pare(Sce_Lexer_Token_Lists* src_ans_token_list) {
	...
	else if (expect_advance_consume_sce_token_lists(src_ans_token_list, E_Sce_Lexer_Punchcute, "{")) {
		return build_sce_ast_array_literal(src_ans_token_list);
	}
